...
import { Sheet, Loader2, CheckCircle, XCircle, ChevronDown, ChevronUp } from 'lucide-react'
...
export default function SheetsImporter({ onSuccess }: Props) {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      setProgress({ current: firstJson.count, total, batches, done: 1 })
      setMessage(`${firstJson.count.toLocaleString()} / ${total.toLocaleString()}건 저장 중...`)

      // 남은 배치 순차 처리
      let uploaded = firstJson.count
